# models/model_2_experimental.py
"""
...
class ExperimentalModel_2_S(BaseTcellModel):
    """
    Experimental-based 50-parameter model.
    ...
    model_name = "Experimental Model - S (50 params)"
    ...
    def __init__(self, n_batch: int = 1, device: torch.device = torch.device("cpu")):
        ...
        self.log_k2 = nn.Parameter(torch.zeros(n_batch, device=device))   # CD3 inhibits IL2 diff
        ...
        self.log_k21 = nn.Parameter(torch.zeros(n_batch, device=device))  # IL4 inhibits IL21 diff (with Hill)
        ...
        self.log_k23 = nn.Parameter(torch.zeros(n_batch, device=device))  # CD3 inhibits IL4 diff
    ...
    def forward(self, t, y):
        ...
        # IL-2: CD28, IL21 promote; CD3, IL4 inhibit differentiation
        dIL2 = (
            k_prod[:, 0]
            + k3 * self.cd28
            + k4 * IL21
            - k2 * self.cd3        # CD3 inhibits differentiation
            - k6 * IL4             # IL4 inhibits differentiation
            - k_deg[:, 0] * IL2
        )
        
        # IFNg: CD3, CD28, IL2 (with Hill), IL2*IL12, IFNg feedback (with Hill), IL12 promote; IL4 inhibits
        dIFNg = (
            k_prod[:, 1]
            + k8 * self.cd3
            + k9 * self.cd28
            ...
            + k13 * IL12_eff
            - k14 * IL4            # IL4 inhibits differentiation
            - k_deg[:, 1] * IFNg
        )
        
        # IL-21: CD3, CD28, IL2 (with Hill), IL21 feedback (with Hill) promote; IL2, IL4 (with Hill) inhibit
        dIL21 = (
            k_prod[:, 2]
            + k15 * self.cd3
            + k16 * self.cd28
            ...
            + k18 * H_act_IL21_k18
            - k20 * IL2            # IL2 inhibits differentiation
            - k21 * IL4 * H_inh_CD3_k21  # IL4 inhibits differentiation (with Hill)
            - k_deg[:, 2] * IL21
        )
        
        # IL-4: Basal (with Hill), CD28, CD28 (with Hill), IL2, IL4 feedback (with Hill) promote; CD3, IFNg, IL12 inhibit
        dIL4 = (
            k_prod[:, 3]
            + k22 * H_inh_CD3_k22
            + k24 * self.cd28
            ...
            - k23 * self.cd3       # CD3 inhibits differentiation
            - k31 * IFNg           # IFNg inhibits differentiation
            - k32 * IL12_eff       # IL12 inhibits differentiation
            - k_deg[:, 3] * IL4
        )
    ...
    def params_to_model(positions, model):
        ...
        with torch.no_grad():
            ...
            # IFNg: k8, k9, k10, k11, k12, k13, k14
            ...
            # IL21: k15, k16, k17, k18, k20, k21
            model.log_k15.data = positions[:, idx]; idx += 1
            model.log_k16.data = positions[:, idx]; idx += 1
            ...
            # IL4: k22, k23, k24, k25, k26, k30, k31, k32
            model.log_k22.data = positions[:, idx]; idx += 1
            model.log_k23.data = positions[:, idx]; idx += 1
            ...
            # Hill K
            for name in ['K10', 'K12', 'K17', 'K18', 'K21', 'K22', 'K25', 'K30']:
                getattr(model, f'log_{name}').data = positions[:, idx]; idx += 1
            
            # Hill h
            for name in ['h10', 'h12', 'h17', 'h18', 'h21', 'h22', 'h25', 'h30']:
